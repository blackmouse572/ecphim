        ...
        <div className="container relative z-30 mx-auto max-w-7xl px-6 pb-16">
          <div className="grid grid-cols-1 lg:grid-cols-[400px_1fr] gap-12 items-end">
            ...
            <MotionList delay={0.2} className="order-1 lg:order-2 space-y-6">
              ...
              <MotionItem>
                <div className="flex flex-wrap items-center gap-4 mb-6">
                  {movie.category.map((cat) => (
                    <span
                      key={cat.id}
                      className="text-xs font-mono font-900 uppercase tracking-[0.4em] text-white/70 border border-white/30 px-4 py-2 rounded-full backdrop-blur-sm hover:bg-white/10 transition-colors"
                    >
                      {cat.name}
                    </span>
                  ))}
                  <div className="flex items-center gap-2 bg-yellow-500/20 border border-yellow-500/40 px-4 py-2 rounded-full">
                    <Star weight="fill" className="h-4 w-4 text-yellow-500" />
                    <span className="font-mono font-700 text-yellow-500">{movie.tmdb.vote_average.toFixed(1)}</span>
                  </div>
                </div>
              </MotionItem>
              ...
              {/* Title */}
              <MotionItem>
                <h1 className="text-display-xl font-100 leading-[0.8] text-white mb-4 tracking-tighter">
                  {movie.name}
                </h1>
                <h2 className="text-title-xl font-200 text-white/70 mb-2">
                  {movie.origin_name}
                </h2>
              ...
              <MotionItem>
                <div className="flex flex-wrap gap-4">
                  <Button
                    size="lg"
                    className="bg-white text-black hover:bg-white/90 font-700 text-lg px-8 py-4 rounded-2xl shadow-2xl hover:scale-105 transition-all"
                  >
                  ...
                  <Button
                    intent="outline"
                    size="lg"
                    className="border-white/30 text-white hover:bg-white/10 font-400 px-8 py-4 rounded-2xl backdrop-blur-sm hover:scale-105 transition-all"
                  >
                  ...
                  <Button
                    ...
                    size=""
                    className="text-white hover:bg-white/10 rounded-2xl p-4 hover:scale-105 transition-all"
                  >
        ...
        <div className="container mx-auto max-w-6xl px-6">
          <div className="grid grid-cols-1 lg:grid-cols-[2fr_1fr] gap-16">
            ...
            <div className="space-y-16">
              {/* Synopsis */}
              <MotionItem>
                <h3 className="text-headline-lg font-900 text-white mb-8 tracking-tight">
                  Synopsis
                </h3>
                <p className="text-body-xl font-200 text-white/80 leading-relaxed">
                  {movie.content}
                </p>
              </MotionItem>
              ...
              <MotionItem>
                <div className="flex items-center justify-between mb-8">
                  <h3 className="text-headline-lg font-900 text-white tracking-tight">
                    Episodes
                  </h3>
                  <div className="text-body font-mono text-white/50">
                  ...
                  {episodes.map((server, serverIndex) => (
                    <MotionItem key={server.server_name} delay={serverIndex * 0.1}>
                      <div className="space-y-4">
                        <h4 className="text-title-lg font-600 text-white/90 flex items-center gap-3">
                        ...
                        <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-3">
                          {server.server_data.map((ep, epIndex) => (
                            <Button
                              key={ep.slug}
                              intent="outline"
                              className="aspect-square border-white/20 text-white hover:border-blue-400 hover:bg-blue-400/10 hover:text-blue-400 transition-all hover:scale-110 rounded-xl font-mono font-700 text-sm"
            ...
            <div className="space-y-12">
              ...
              <MotionItem delay={0.3}>
                <div className="bg-white/[0.02] border border-white/10 rounded-3xl p-8 backdrop-blur-sm">
                  <h4 className="text-title-lg font-900 text-white mb-6">Details</h4>
              ...
              <MotionItem delay={0.5}>
                <div className="bg-gradient-to-br from-blue-500/10 to-cyan-500/10 border border-blue-500/20 rounded-3xl p-8 backdrop-blur-sm">
                  <div className="grid grid-cols-3 gap-4 text-center">
                    <div>
                      <div className="text-title-lg font-900 text-white">{movie.view}</div>
                      <div className="text-xs font-mono uppercase tracking-wider text-white/50">Views</div>
                    </div>
                    <div>
                      <div className="text-title-lg font-900 text-white">{movie.tmdb.vote_count.toLocaleString()}</div>
                      <div className="text-xs font-mono uppercase tracking-wider text-white/50">Votes</div>
                    </div>
                    <div>
                      <div className="text-title-lg font-900 text-yellow-400">{movie.tmdb.vote_average}</div>
