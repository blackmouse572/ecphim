...
const MOVIES = Array.from({ length: 20 }, (_, i) => ({
  ...
  category: ["Action", "Drama", "Thriller", "Comedy"][Math.floor(Math.random() * 4)],
  episodes: Math.floor(Math.random() * 24) + 1,
  duration: `${Math.floor(Math.random() * 60) + 90} min`,
  description: "An epic tale of adventure, betrayal, and redemption that will keep you on the edge of your seat from beginning to end.",
}));

const FILTER_TAGS = [
  "All", "Action", "Drama", "Comedy", "Horror", "Sci-Fi", "Romance", "Thriller"
];

export default function BrowsePage({ params }: { params: { category?: string } }) {
  const category = params.category || 'trending';
  const categoryInfo = CATEGORY_DATA[category as keyof typeof CATEGORY_DATA] || CATEGORY_DATA.trending;
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-mono font-700 text-yellow-500">{MOVIES[0].rating}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-4 text-sm text-white/70">
                      ...
                      <span className="font-mono uppercase tracking-wider">{MOVIES[0].category}</span>
                    </div>
